...
def byte_to_binary(byte):
    ...
    '0000000001001010'
    '''
    return ''.join(str((byte & (1 << i)) and 1) for i in reversed(list(range(8))))
...
def dict_to_csv(items, delimiter, header):
    ...
    if len(items) > 0:
        delimiter = to_char(delimiter)
        output = StringIO()
        csvwriter = csv.DictWriter(output, fieldnames=list(items[0].keys()),
                                   delimiter=delimiter)
        if header:
            csvwriter.writerow(dict((key, key) for key in list(items[0].keys())))
